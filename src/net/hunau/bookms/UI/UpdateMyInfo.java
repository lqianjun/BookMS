...
public class UpdateMyInfo extends JFrame {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					UpdateMyInfo frame = new UpdateMyInfo(user);
	...
	public UpdateMyInfo(User user) {
		...
		C_readerType.setEditable(false);
...
/**************************************************【对输入数据进行格式检查start】************************************************************/
			if(C_username.getText().isEmpty()) {		//判断用户名是否为空
				JOptionPane.showMessageDialog(UpdateMyInfo.this, new String("请输入用户名！"));return;
			}
			if((new String(C_password.getPassword())).isEmpty())
			{
				JOptionPane.showMessageDialog(UpdateMyInfo.this, new String("请输入密码！"));return;
			}
			if((new String(C_rePassword.getPassword())).isEmpty())
			{
				JOptionPane.showMessageDialog(UpdateMyInfo.this, new String("请输入二次密码！"));return;
			}
			//判断两次密码是否一致
			...
			if(C_userNo.getText().isEmpty()) {
				JOptionPane.showMessageDialog(UpdateMyInfo.this, new String("请输入学号！"));return;
			}
			if(C_readerType.getText().isEmpty()) {
				JOptionPane.showMessageDialog(UpdateMyInfo.this, new String("请输入身份！"));return;
			}
/**************************************************【对输入数据进行格式检查end】************************************************************/
			UserDAO ud=new UserDAO();
			...
			JOptionPane.showMessageDialog(UpdateMyInfo.this, "更新成功！");
			
		 }
		});
		btnNewButton.setBounds(128, 427, 63, 32);
		...
		button.setBounds(220, 427, 63, 32);
