...
public class AddBook extends JFrame {
	...
	/**************************************************【对输入数据进行格式检查start】************************************************************/
				if(C_barcode.getText().isEmpty()) {		//判断用户名是否为空
					JOptionPane.showMessageDialog(AddBook.this, new String("请输入书籍条码！"));return;
				}
				if(C_bookname.getText().isEmpty())
				{
					JOptionPane.showMessageDialog(AddBook.this, new String("请输入书名！"));return;
				}
				if(C_author.getText().isEmpty())
				{
					JOptionPane.showMessageDialog(AddBook.this, new String("请输入作者！"));return;
				}
				if(C_bookTypeId.getText().isEmpty()) {
					JOptionPane.showMessageDialog(AddBook.this, new String("请输入类型编码！"));return;
				}
				if(C_location.getText().isEmpty()) {
					JOptionPane.showMessageDialog(AddBook.this, new String("请输入存放位置！"));return;
				}
				if(C_tbarcode.getText().isEmpty()) {
					JOptionPane.showMessageDialog(AddBook.this, new String("请输入一级编号！"));return;
				}
				if(C_damage.getText().isEmpty()) {
					JOptionPane.showMessageDialog(AddBook.this, new String("请输入新旧程度！"));return;
				}
	/**************************************************【对输入数据进行格式检查end】************************************************************/
				...
				try {
					...
					{
						JOptionPane.showMessageDialog(AddBook.this, "请输入正确的书籍类型编号！");return;
					}
				...
				book.setLend(0);//新书入库，0表示未借出
				try {
					book.setPrice(Float.parseFloat(C_price.getText().trim()));
				}
				catch(NumberFormatException e)
				{
					JOptionPane.showMessageDialog(AddBook.this, "非法输入，请在价格栏输入数值!");return;
				}
				String temp = new String(C_publishDate.getText().trim());
				...
				try {
					...
					// TODO Auto-generated catch block
					JOptionPane.showMessageDialog(AddBook.this, "请输入正确的日期,格式为yyyy-MM-dd！");
					e1.printStackTrace();
					return;
				}
				...
				book.setPublish(C_publish.getText().trim());
				try {
					//防止重复添加书籍
					if(bd.getBookInfoByBarcode(book.getBarcode()) != null)
					{
						JOptionPane.showMessageDialog(AddBook.this,"请勿重复添加书籍！");
					}
					bd.addBook(book);
					//访问数据库，查找是否已添加此书
					if(bd.getBookInfoByBarcode(book.getBarcode())==null)
					{
						JOptionPane.showMessageDialog(AddBook.this, "添加失败！");
					}
					else
					{
						JOptionPane.showMessageDialog(AddBook.this, "添加成功！");
					}
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
